...
export function Header({ currentView, setCurrentView }: HeaderProps) {
  ...
  const getTabValue = () => {
    ...
    if (currentView === "challenges" || currentView === "add-challenge")
      return "challenges";
  ...
  return (
    ...
    >
      ...
      <div className="sm:hidden fixed bottom-0 left-0 right-0 bg-background/95 backdrop-blur-sm border-t border-border p-2">
        ...
        >
          <TabsList className="grid grid-cols-3 h-15 w-full">
            <TabsTrigger
              value="dashboard"
              className="flex flex-col justify-around items-center gap-1"
            >
              <Home className="h-4 w-4" />
              <span>Dashboard</span>
            </TabsTrigger>
